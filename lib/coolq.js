 ...
 * @param md5
 */
const getImageUrlByMd5 = (md5) => 'https://gchat.qpic.cn/gchatpic_new/0/0-0-' + md5.toUpperCase() + '/0'
        ...
        text = message.replace(/\n/gu, '&#10;').replace(/\[CQ:([^,]*?)\]/gu, '[CQ:$1,]').replace(/\[CQ:(.*?)((?:,).*?)\]/gu, (_, type, param) => {
            ...
            switch (type) {
                ...
                case 'image':
                    ...
                    if (tmp && tmp[1]) {
                        let md5 = tmp[1].substr(0, 32);
                        /^([a-f\d]{32}|[A-F\d]{32})$/.test(md5) && (tmp1 = getImageUrlByMd5(md5));
                        images.push(tmp1);
                    }
                    return tmp3 ? '[闪照]' : '[图片]';
            ...
            switch (curr.type) {
                ...
                case 'image':
                    if (curr.data.url) {
                        ...
                        let md5 = file.substr(0, 32);
                        /^([a-f\d]{32}|[A-F\d]{32})$/.test(md5) && (tmp = getImageUrlByMd5(md5));
                        images.push(tmp);
                    }
                    text = tmp2 ? '[闪照]' : '[图片]';
                    break;
                ...
                case 'face':
                    text += `[${faces[curr.data.id] || '表情#'+curr.data.id}]`;
                    break;
