...
class QQOICQMessageHandler extends MessageHandler {
    constructor(config = {}) {
        ...
        this._online_status = botConfig.online_status || 11;
        ...
        client.on('system.online', ()=>{
            client.setOnlineStatus(this._online_status);
    ...
    getNick(message) {
        if (message) {
            let { user_id, nickname, card,    qq, name, groupCard, } = message.sender;
            user_id = user_id || qq || '';
    ...
    getTitle(message) {
        if (message) {
            let { title,  honor } = message.sender;
